...
v2 是固定上下文、单轮、无工具的 model-only 评测。`common-32k` 在统一 Chat Template 后使用 28672 token 输入预算（为输出保留 4096 token），确定性裁剪 Diff，并记录 token/字符数、截断原因、template/user/messages SHA；`native-context` 仅用于诊断，不应与主比较混用。Agentic 分支仍保持原有工具循环和 Docker 评测逻辑。
