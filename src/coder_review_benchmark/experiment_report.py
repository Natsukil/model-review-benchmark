...
def generate_experiment_report(experiment_dir: Path, output_dir: Path | None = None) -> dict[str, Path]:
    ...
    for suite, data in suites.items():
        ...
        payload = {"experiment_id": experiment_id, "suite": suite, "runs": data["runs"], "per_sample": data["samples"]}
        ...
        lines = [f"# {suite} report", "", f"Experiment: `{experiment_id}`", "", "| Model | Dataset | Status | Samples | " + " | ".join(headers) + " |", "|---|---|---|---:|" + "---:|" * len(fields)]
        for row in data["runs"]:
            values = [str(row.get(key, "")) for key, _ in fields]
    ...
    complete = {"experiment_id": experiment_id, "state": state, "manifests": manifests, "judge_manifest": _read_json(judge_manifest_path) if judge_manifest_path.exists() else None, "runs": all_summary, "suites": {suite: {"run_count": len(data["runs"]), "sample_count": len(data["samples"])} for suite, data in suites.items()}}
    ...
    _write_excel(output_dir / "report.xlsx", all_summary, all_samples)
    md_lines = ["# Model Review Benchmark experiment", "", f"Experiment: `{experiment_id}`", "", "| Suite | Model | Dataset | Status | Samples | Errors |", "|---|---|---|---|---:|---:|"]
